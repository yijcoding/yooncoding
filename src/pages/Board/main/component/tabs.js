    ...
    return (
        ...
        <div>
                ...
                <div>
                    <ul className="tabs">
                        {tabEvent.map((el, index) => (
                            <Link key={el.id} to={`/board?b_type=${el.link}&viewCnt=${pageCount}`}>
                                <li className={index === data ? "tab-link current" : "tab-link"}
                                    id={el.id}
            ...
            <div className="menu_select" >
                <div className="text" >
                    <select className="form-select form-select-sm" id="view-select" aria-label=".form-select-sm example" onChange={handlePagecountChange}>
                        {pagecount.map((sl) => {
