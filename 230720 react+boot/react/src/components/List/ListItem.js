...
const ListItem = ({ item, member_id, handleShowToast }) => {
  ...
  useEffect(() => {
    if (!member_id) return;
    axios
      .post("http://localhost:8080/selectedapi/check", {
  ...
  const handleHeartClick = () => {
    ...
    axios
      .post("http://localhost:8080/selectedapi/insertDelete", {
        amuse_id: item.amuse_id,
        member_id: member_id,
      })
      .then((response) => {
    ...
    <Col xs={12} md={6} lg={4} xl={3} key={item.amuse_id} className="mb-5">
      <Card className="h-100">
        <div className="position-relative overflow-hidden">
          <div
            className={`animate__animated position-absolute ${
              animateHeart ? "animate__bounce" : ""
            }`}
            onClick={handleHeartClick}
            style={{ width: "35px", top: "10px", right: "10px", zIndex: "100" }}
          >
            {isFavorite ? (
              <BsHeartFill
          ...
          <Card.Link href={`amuseDetail?amuse_id=${item.amuse_id}`}>
            <Card.Img className="custom-card-img" src={item.url} alt="..." />
          </Card.Link>
          <div
            data-id={item.amuse_id}
            className="animate__animated position-absolute heart-wrap"
